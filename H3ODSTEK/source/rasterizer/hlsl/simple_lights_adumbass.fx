...
// 5/15/2024 4:47 pm Amit this isnt going to work the index changes as you move the camera. maybe massive bounding radius
/*
void calculate_simple_light_ibl(
		uniform int light_index,
		in float3 fragment_position_world,
		out float3 light_position,
		out float light_falloff,
        out float probe_index)			// return normalized direction to the light
{
#ifdef dynamic_lights_use_array_notation
#define		LIGHT_DATA(offset, registers)	(SIMPLE_LIGHT_DATA[light_index][(offset)].registers)
#elif DX_VERSION == 9
...
#define		LIGHT_BOUNDING_RADIUS	LIGHT_DATA(4, x)
	...
	// calculate direction to light (4 instructions)
	light_position= LIGHT_POSITION - fragment_position_world;				// vector from fragment to light
	float  light_dist2= dot(light_position, light_position);				// distance to the light, squared
	light_position  *=rsqrt(light_dist2);									// normalized vector pointing to the light
		
	float2 falloff;
	falloff.x= 1 / (LIGHT_SIZE + light_dist2);									// distance based falloff				(2 instructions)
	falloff.y= dot(light_position, LIGHT_DIRECTION);							// angle based falloff (spot-light)		(1 instruction)
	falloff= max(0.0f, falloff * LIGHT_FALLOFF_SCALE + LIGHT_FALLOFF_OFFSET);	// scale, offset, clamp result			(2 instructions)
	falloff.y= pow(falloff.y, LIGHT_SMOOTH) + LIGHT_SPHERE;						// smooth and add ambient				(4 instructions)
	float combined_falloff= saturate(falloff.x) * saturate(falloff.y);								//										(1 instruction)
    light_falloff = falloff.y;
    light_position =LIGHT_COLOR * combined_falloff;	
    probe_index = light_index;
}

void calc_simple_lights_ibl(
		in float3 fragment_position_world,
		...
		out float3 light_position,
		out float light_falloff,
        out float probe_index)


{
	int light_index= 0;
    light_position=  float3(0.0f, 0.0f, 0.0f);
	light_falloff= 0.0f;
    probe_index = LIGHT_SPHERE;
	
	// add in simple lights
//#ifndef pc	
	[loop]
//#endif
	for ( light_index= 0; light_index < SIMPLE_LIGHT_COUNT; light_index++)
	{
		...
		calculate_simple_light_ibl(
			light_index, fragment_position_world, light_position, light_falloff, probe_index);
