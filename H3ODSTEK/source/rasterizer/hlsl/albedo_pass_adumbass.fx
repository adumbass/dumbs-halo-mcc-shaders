...
albedo_pixel convert_to_albedo_target(in float4 albedo, in float3 normal, in float pos_w)
{
    ...
    int decal_mask = 0.0; 
    int ibl_mask = 1.0;
    // Combine the states into a single 2-bit value
    int combined_mask = (decal_mask & 0x01) | ((ibl_mask & 0x01) << 1);
    // Normalize the combinedState to the range [0, 1] for the alpha channel
    float packed_mask = combined_mask / 3.0f;
 

	result.normal.w= packed_mask;//albedo.w;
