...
float3 calc_self_illumination_simple_ps(
	in s_shader_data SHADER_DATA)
{
	float4 result= sample2D(self_illum_map, transform_texcoord(SHADER_DATA.common.texcoord, self_illum_map_xform)) * self_illum_color;		// ###ctchou $PERF roll self_illum_intensity into self_illum_color
...
float3 calc_self_illumination_plasma_ps(
	in s_shader_data SHADER_DATA)
{
	float alpha=	sample2D(alpha_mask_map, transform_texcoord(SHADER_DATA.common.texcoord, alpha_mask_map_xform)).a;
	float noise_a=	sample2D(noise_map_a, transform_texcoord(SHADER_DATA.common.texcoord, noise_map_a_xform)).r;
	float noise_b=	sample2D(noise_map_b, transform_texcoord(SHADER_DATA.common.texcoord, noise_map_b_xform)).r;
...
float3 calc_self_illumination_three_channel_ps(
	in s_shader_data SHADER_DATA)
{
	float4 self_illum= sample2D(self_illum_map, transform_texcoord(SHADER_DATA.common.texcoord, self_illum_map_xform));
...
float3 calc_self_illumination_from_albedo_ps(
	in s_shader_data SHADER_DATA)
{
	float3 self_illum= SHADER_DATA.albedo*self_illum_color.xyz*self_illum_intensity;
	SHADER_DATA.albedo.rgb= float3(0.f, 0.f, 0.f);
...
{
	float4 self_illum=			sample2D(self_illum_map,			transform_texcoord(SHADER_DATA.common.texcoord, self_illum_map_xform));
	float4 self_illum_detail=	sample2D(self_illum_detail_map,		transform_texcoord(SHADER_DATA.common.texcoord, self_illum_detail_map_xform));
...
float3 calc_self_illumination_meter_ps(
	in s_shader_data SHADER_DATA)
{
	float4 meter_map_sample= sample2D(meter_map, transform_texcoord(SHADER_DATA.common.texcoord, meter_map_xform));
...
float3 calc_self_illumination_times_diffuse_ps(
	in s_shader_data SHADER_DATA)
{
	float3 self_illum_texture_sample= sample2D(self_illum_map, transform_texcoord(SHADER_DATA.common.texcoord, self_illum_map_xform)).rgb;
	
	float albedo_blend= max(self_illum_texture_sample.g * 10.0 - 9.0, 0.0);
	float3 albedo_part= albedo_blend + (1-albedo_blend) * SHADER_DATA.albedo;
...
float3 calc_self_illumination_palette_ps(
	in s_shader_data SHADER_DATA)
{
	float illum_index= sample2D(illum_index_map, transform_texcoord(SHADER_DATA.common.texcoord, illum_index_map_xform)).x;
	...
	in s_shader_data SHADER_DATA)
{
	...
	SHADER_DATA.common.view_dir= -SHADER_DATA.common.view_dir;
	...
	float2 wall_coordinate_xy=	(floor(SHADER_DATA.common.texcoord.xy * transform_xform.xy + transform_xform.zw + 0.5f + sign(SHADER_DATA.common.view_dir.xy) * 0.5f) - transform_xform.zw) / transform_xform.xy;
	
	// calculate the distance each of the walls
	float2 distance_xy=			(wall_coordinate_xy - SHADER_DATA.common.texcoord.xy) / SHADER_DATA.common.view_dir.xy;

	// we hit the closest wall first.  calculate the intersection coordinates
	float3 intersection=		float3(SHADER_DATA.common.texcoord.xy, 0) + min(distance_xy.x, distance_xy.y) * SHADER_DATA.common.view_dir.xyz;
	
	// sample our per-window property map (tint r,g,b and texture offset used to select room properties)
//	float4 window=				sample2D(window_property_map, transform_texcoord(SHADER_DATA.common.texcoord, window_property_map_xform));
...
//		}
	...
	{
		if (SHADER_DATA.common.view_dir.y > 0)
		{
...
/*
	...
	index= index + index * (SHADER_DATA.common.view_dir.y > 0);
...
*/
	...
	float4 opacity=		sample2D(opacity_map, transform_texcoord(SHADER_DATA.common.texcoord, opacity_map_xform));
