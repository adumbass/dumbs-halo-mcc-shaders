...
float3 calc_environment_map_per_pixel_ps(
	in s_shader_data SHADER_DATA)
{
	SHADER_DATA.common.view_reflect_dir.y= -SHADER_DATA.common.view_reflect_dir.y;
	
	float4 reflection;
#ifdef pc	
	reflection= sampleCUBE(environment_map, SHADER_DATA.common.view_reflect_dir);
#else
	reflection= sampleCUBElod(environment_map, SHADER_DATA.common.view_reflect_dir, 0.0f);
#endif
	return reflection.rgb * SHADER_DATA.envmap_specular_reflectance_and_roughness.xyz * SHADER_DATA.envmap_area_specular_only * env_tint_color * reflection.a;
}
	...
	in s_shader_data SHADER_DATA)
{
...
#ifdef pc
	float grad_x= length(ddx(SHADER_DATA.common.view_reflect_dir));
	float grad_y= length(ddy(SHADER_DATA.common.view_reflect_dir));
	float base_lod= 6.0f * sqrt(max(grad_x, grad_y)) - 0.6f;
	float lod= max(base_lod, SHADER_DATA.envmap_specular_reflectance_and_roughness.w * env_roughness_scale * 4);
	
	reflection_0= sampleCUBElod(dynamic_environment_map_0, SHADER_DATA.common.view_reflect_dir, lod);
	reflection_1= sampleCUBElod(dynamic_environment_map_1, SHADER_DATA.common.view_reflect_dir, lod);
	...
	return reflection * SHADER_DATA.envmap_specular_reflectance_and_roughness.xyz * env_tint_color * SHADER_DATA.envmap_area_specular_only;
	...
	in s_shader_data SHADER_DATA)
{
	...
	float4 reflection_0= texCUBEbias(dynamic_environment_map_0, float4(SHADER_DATA.common.view_reflect_dir, SHADER_DATA.envmap_specular_reflectance_and_roughness.w + env_bias)); 
	float4 reflection_1= texCUBEbias(dynamic_environment_map_1, float4(SHADER_DATA.common.view_reflect_dir, SHADER_DATA.envmap_specular_reflectance_and_roughness.w + env_bias)); 
	...
	float4 topcoat_0= texCUBEbias(dynamic_environment_map_0, float4(SHADER_DATA.common.view_reflect_dir, env_topcoat_bias)); 
	float4 topcoat_1= texCUBEbias(dynamic_environment_map_1, float4(SHADER_DATA.common.view_reflect_dir, env_topcoat_bias)); 
	...
	return ((reflection * SHADER_DATA.envmap_specular_reflectance_and_roughness.xyz * env_tint_color) + (topcoat * env_topcoat_color)) * SHADER_DATA.envmap_area_specular_only;
}
*/
	...
	in s_shader_data SHADER_DATA)
{
	...
	// fisheye projection	
	float3 envmap_dir;
	envmap_dir.x= dot(SHADER_DATA.common.view_reflect_dir, flat_envmap_matrix_x.xyz);
	envmap_dir.y= dot(SHADER_DATA.common.view_reflect_dir, flat_envmap_matrix_y.xyz);
	envmap_dir.z= dot(SHADER_DATA.common.view_reflect_dir, flat_envmap_matrix_z.xyz);
	...
	float2 SHADER_DATA.common.texcoord= envmap_dir.xy * radius / sqrt(dot(envmap_dir.xy, envmap_dir.xy));		// normalize x/y vector, and scale by radius to get fisheye projection
	SHADER_DATA.common.texcoord= (1.0f+SHADER_DATA.common.texcoord)*0.5f;															// convert to texture coordinate space (0..1)

#if ENVMAP_TYPE(envmap_type) == ENVMAP_TYPE_from_flat_texture_as_cubemap
	float3 cube_texcoord = float3(
		1.0f,
		-((SHADER_DATA.common.texcoord.y * 2.0f) - 1.0f),
		-((SHADER_DATA.common.texcoord.x * 2.0f) - 1.0f));
	reflection = sampleCUBE(flat_environment_map, cube_texcoord);
#else
	reflection= sample2D(flat_environment_map, SHADER_DATA.common.texcoord);
#endif
...
/*	// perspective projection
	float3 SHADER_DATA.common.texcoord;
	SHADER_DATA.common.texcoord.z= dot(SHADER_DATA.common.view_reflect_dir, flat_envmap_matrix_w.xyz);					// ###ctchou $TODO $PERF pass the transformed point from the vertex shader
	if (SHADER_DATA.common.texcoord.z < -0.001f)
	{
		SHADER_DATA.common.texcoord.x= dot(SHADER_DATA.common.view_reflect_dir, flat_envmap_matrix_x.xyz);
		SHADER_DATA.common.texcoord.y= dot(SHADER_DATA.common.view_reflect_dir, flat_envmap_matrix_y.xyz);
		reflection= sample2D(flat_environment_map, SHADER_DATA.common.texcoord.xy / SHADER_DATA.common.texcoord.z);
	}
*/
	return reflection * SHADER_DATA.envmap_specular_reflectance_and_roughness.xyz * env_tint_color;
}

float3 calc_environment_map_from_flat_texture_as_cubemap_ps(
	in s_shader_data SHADER_DATA)
{
	calc_environment_map_from_flat_texture_ps(SHADER_DATA);
}
