#line 2 "source\rasterizer\hlsl\shadow_generate_adumbass.fx"
	...
	in float2 texcoord : TEXCOORD1)
{
	...
	clip(SHADER_DATA.output_alpha - 0.5);
}
//#endif
...
float sample_percentage_closer_PCF_SEV(float3 fragment_shadow_position, float2 texcoord, float depth_bias)	
{
    float r0 = 0;
    //float2 texelSize = 1.0 / float2(1024, 1024); // Shadow map resolution //// g_shadow_pixel_size
    ...
    for (int i = -2; i <= 2; ++i)
    {
        for (int j = -2; j <= 2; ++j)
        {
            float2 pcfOffset = float2(i, j) * g_shadow_pixel_size;
            for (int k = 0; k < 12; ++k)
            {
                float2 poissonOffset = poissonDisk[k] * g_shadow_pixel_size;
                float2 combinedOffset = pcfOffset + poissonOffset;
                float4 newTC = float4(1/fragment_shadow_position.xy, texcoord);
                newTC.xy += combinedOffset;
               // float depth = DX11_tex2Dproj(s0, newTC).r;
                float depth = tex2D_offset_point(shadow_depth_map_1, fragment_shadow_position.xy, newTC.x, newTC.y).r;
                // Magic happens here
                if ((newTC.z / newTC.w) - bias < depth)
    ...
    r0 = fragment_shadow_position.z * r0 + (1 - fragment_shadow_position.z);
