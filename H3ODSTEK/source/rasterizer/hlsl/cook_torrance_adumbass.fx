...
PARAM(bool, use_albedo_as_spec_tint);
	...
	*/
{
	...
	// a: roughless
	SHADER_DATA.spatially_varying_material_parameters= float4(specular_coefficient, albedo_blend, environment_map_specular_contribution, roughness);
	...
	SHADER_DATA.specular_albedo_color = SHADER_DATA.albedo.rgb * SHADER_DATA.spatially_varying_material_parameters.g + fresnel_color * (1-SHADER_DATA.spatially_varying_material_parameters.g);

	float n_dot_l = dot( SHADER_DATA.bump_normal, SHADER_DATA.dominant_light_direction );
	float n_dot_v = dot( SHADER_DATA.bump_normal, SHADER_DATA.common.view_dir );
    float min_dot = min( n_dot_l, n_dot_v );
    
    if ( min_dot > 0)
	{
        // compute geometric attenuation
        float3 half_vector = normalize( SHADER_DATA.common.view_dir + SHADER_DATA.dominant_light_direction );
        float n_dot_h = dot( SHADER_DATA.bump_normal, half_vector );
        float v_dot_h = dot( SHADER_DATA.common.view_dir, half_vector); 
		...
		float3 f0= min(SHADER_DATA.specular_albedo_color, 0.999f);
		...
		SHADER_DATA.specular_fresnel_color= ( 0.5f * ( (gmc*gmc) / (gpc*gpc + 0.00001f) ) * ( 1.f + r*r ));
		
		//calculate the distribution term
		float t_roughness= max(SHADER_DATA.spatially_varying_material_parameters.a, 0.05f);
		...
		SHADER_DATA.analytic_specular_radiance= distribution * saturate(geometry_term) / (3.14159265 * n_dot_v + 0.00001f) * SHADER_DATA.specular_fresnel_color;
		SHADER_DATA.analytic_specular_radiance= min(SHADER_DATA.analytic_specular_radiance, vertex_n_dot_l + 1.0f) * SHADER_DATA.dominant_light_intensity;
		
	}
	else
	{
		SHADER_DATA.analytic_specular_radiance= 0.00001f;
		SHADER_DATA.specular_fresnel_color= SHADER_DATA.specular_albedo_color;
	}
}
...
{  
	...
	/*
		...
		*/
		...
		float r_dot_l= max(dot(SHADER_DATA.dominant_light_direction, SHADER_DATA.common.view_reflect_dir), 0.0f) * 0.65f + 0.35f;
		...
		sh_glossy= specular_part * SHADER_DATA.specular_albedo_color + (1 - SHADER_DATA.specular_albedo_color) * schlick_part;
		SHADER_DATA.envmap_specular_reflectance_and_roughness.w= SHADER_DATA.spatially_varying_material_parameters.a;
		SHADER_DATA.envmap_area_specular_only= sh_glossy * SHADER_DATA.prt_ravi_diff.z * specular_tint;
				
		//scaling and masking
		
		SHADER_DATA.specular_radiance.xyz= SHADER_DATA.specular_mask * SHADER_DATA.spatially_varying_material_parameters.r * specular_tint * (
			(SHADER_DATA.simple_light_specular_light * SHADER_DATA.specular_albedo_color + SHADER_DATA.analytic_specular_radiance) * analytical_specular_contribution +
			max(sh_glossy, 0.0f) * area_specular_contribution);
			
		SHADER_DATA.specular_radiance.w= 0.0f;
		
		SHADER_DATA.envmap_specular_reflectance_and_roughness.xyz=	SHADER_DATA.spatially_varying_material_parameters.b * SHADER_DATA.specular_mask * SHADER_DATA.spatially_varying_material_parameters.r;		// ###ctchou $TODO this ain't right
				
		float diffuse_adjusted= diffuse_coefficient;
		if (use_material_texture)
		{
			diffuse_adjusted= 1.0f - SHADER_DATA.spatially_varying_material_parameters.r;
		}
			
		SHADER_DATA.diffuse_radiance= SHADER_DATA.diffuse_radiance * SHADER_DATA.prt_ravi_diff.x;
		SHADER_DATA.diffuse_radiance= (SHADER_DATA.simple_light_diffuse_light + SHADER_DATA.diffuse_radiance) * diffuse_adjusted;
		SHADER_DATA.specular_radiance*= SHADER_DATA.prt_ravi_diff.z;		
		
		//diffuse_color= 0.0f;
		//SHADER_DATA.specular_radiance= spatially_varying_material_parameters.r;
	}
#if defined(pc) && (DX_VERSION == 9)
	else
	{
		SHADER_DATA.envmap_specular_reflectance_and_roughness= float4(0.f, 0.f, 0.f, 0.f);
		SHADER_DATA.envmap_area_specular_only= float3(0.f, 0.f, 0.f);
		SHADER_DATA.specular_radiance= 0.0f;
		SHADER_DATA.diffuse_radiance= ravi_order_3(SHADER_DATA.bump_normal, sh_lighting_coefficients) * SHADER_DATA.prt_ravi_diff.x;
	}
#endif // pc
}

PARAM_SAMPLER_2D(spec_tint_map);
PARAM(float4, spec_tint_map_xform);
...
{
	float3 spec_tint;
	if (use_albedo_as_spec_tint)
	{
		spec_tint = SHADER_DATA.albedo.rgb;
	}
	else
	{
...
#endif // pc
	{
		...
		sh_glossy = specular_part * SHADER_DATA.specular_albedo_color  + (1 - SHADER_DATA.specular_albedo_color ) * schlick_part;
		SHADER_DATA.envmap_specular_reflectance_and_roughness.w = SHADER_DATA.spatially_varying_material_parameters.a;
		SHADER_DATA.envmap_area_specular_only = sh_glossy * SHADER_DATA.prt_ravi_diff.z * spec_tint;

		//scaling and masking

		SHADER_DATA.specular_radiance.xyz = SHADER_DATA.specular_mask * SHADER_DATA.spatially_varying_material_parameters.r * spec_tint * (
			(SHADER_DATA.simple_light_specular_light * SHADER_DATA.specular_albedo_color  + SHADER_DATA.analytic_specular_radiance) * analytical_specular_contribution +
			max(sh_glossy, 0.0f) * area_specular_contribution);
